...
It is noticeable that most devices exhibit hyperthreading (multiple threads
active simultaneously per physical core), either at the compute element like
Nvidia warp which are 8-way SIMD with a 4x hyperthreading, or at the compute
unit like the Xeon processors (2x hyperthreading, in our case desactivated at
the BIOS level) and accelerator (4x hyperthreading). Hyperthreading is a way
hardware manufacturers found to hide the memory latency by having more threads
 ...
 * Scatter: write multiple output from a single input (needs atomic operation
   support)
...
Using PyOpenCL, it is possible to program the same way CPUs and GPUs. By taking
into account the SIMD side of CPU cores, their programming looks pretty similar
to GPUs or MIC accelerators and this allows to write a single code which takes
benefit of the whole processing power of any of those devices (while excluding
the prefetcher from CPU).
